...
/**
 * Recommended base class to implement a relationship document making use of
 ...
 * <b>ALL</b> of the methods. Makes use of the source and target document
 * document to implement the relationship features. Modification are
 * implemented by fetching the target resources, adding them the the source
 * document with reflection and then saving the source document. Lookup is
 * implemented by querying the target document document and filtering in the
 ...
 * fashion. Setting a relation on a document and saving it assumes that the save
	...
	private void handleTarget(MultivaluedMap<I, D> bulkResult, D result, Set<I> sourceIdSet, String oppositeName, ResourceInformation sourceInformation) {
		...
		if (property instanceof Iterable) {
			for (T potentialSource : (Iterable<T>) property) {
				...
				// for to-many relations we have to assigned the found document
