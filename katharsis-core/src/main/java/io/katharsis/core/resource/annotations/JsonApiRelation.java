...
import static io.katharsis.core.resource.annotations.LookupIncludeBehavior.NONE;
import static io.katharsis.core.resource.annotations.SerializeType.LAZY;
...
public @interface JsonApiRelation {
	...
	/**
	 * (Optional) This attribute is used to make automatic value assignment using a defined relationship document if such document
	 * is available.
	 *
	...
	/**
	 * @return opposite attribute name in case of a bidirectional association. Used by {@link io.katharsis.core.repository.RelationshipRepositoryBase} to implement
	 * its findOneTarget and findManyTarget functions by directly searching in the related document document with a filter in the opposite direction.
	 * Allow to work with relations with only implementing document repositories!
	 */
